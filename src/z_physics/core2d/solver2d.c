...
void zp_manifold2d_soft_prepare_contact(zp_manifold2d *const zp_restrict m, void *const zp_restrict w, const zp_solver_input2d *const input) {
 ...
 for(uint8_t i = 0; i < m->_contact_count; i++) {
  ...
  float restitution_threshold = zp_sqrt(zp_dot2(world->_gravity, world->_gravity)) * input->_dt;
...
void zp_manifold2d_soft_solve_contact(zp_manifold2d *const zp_restrict m, void *const zp_restrict w, const zp_solver_input2d *const input) {
 ...
 for(uint8_t i = 0; i < m->_contact_count; i++) {
  ...
  float substeps = (float)input->_solver_substeps;
  float bias_factor = 1.0f / substeps;
  float slop = 0.004f;
  float penetration_error = 0.0f;
 
  if(depth < 0.0f) {
#ifdef ZP_USE_EXPONENTIAL_SCALE_PENETRATION_ERROR
  /* exponential scale */
  const float log2_e = -1.44269504f;
  depth = -(zp_exp2(depth * log2_e) - 1.0f);
#endif
   float error = zp_min((contact->_depth + slop), 0.0f);
   penetration_error = -error * bias_factor * input->_bias_coeff;
  } else {
   penetration_error = contact->_depth * input->_inv_dt;
  }

#ifdef ZP_USE_LOGARITHMIC_DAMPING_PENETRATION_ERROR
  /* logarithmic with damping-like behaviour */
  const float inv_log2_e = 0.69314718f;
  penetration_error = zp_log2(penetration_error + 1.0f) * inv_log2_e;
#endif
  ...
  if(vn < contact->_bias) {
   ...
   penetration_error = -vn + penetration_error;
	 ...
	 j = input->_mass_coeff * contact->_mass_normal * penetration_error;
